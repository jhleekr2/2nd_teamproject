...
//하지만 여기서는 게시글과 댓글 페이징에 여러 차이가 있기 때문에 페이징 처리 VO를 분리함
@Data
public class Paging {
	...
	private int memberno; //회원번호(게시글 작성자 번호)
...
//	}
	...
	//게시글 작성자 번호 반영한 생성자 만들기
	public Paging(int curPage, int totalCount, int memberno) {
		super();
		this.curPage = curPage;
		this.totalCount = totalCount;
	...
	public Paging(int curPage, int totalCount, int listCount, int pageCount, int memberno) {
		super();
		this.curPage = curPage;
		this.totalCount = totalCount;
		this.listCount = listCount;
	...
	//검색결과, 게시글 작성자 번호 반영한 생성자 만들기
	public Paging(int curPage, int totalCount, String search, int memberno) {
		super();
		this.curPage = curPage;
		this.totalCount = totalCount;
		this.search = search;
	...
	public Paging(int curPage, int totalCount, int listCount, int pageCount, String search, int memberno) {
		super();
		this.curPage = curPage;
		this.totalCount = totalCount;
		this.listCount = listCount;
...
//		if( listCount == 0 ) setListCount(10); //화면에 보여질 글 수 10개
		if( listCount == 0 ) setListCount(3); //화면에 보여질 글 수 3개
//		if( pageCount == 0 ) setPageCount(10); //화면에 보여질 페이지 수 10개
		if( pageCount == 0 ) setPageCount(3); //화면에 보여질 페이지 수 3개
